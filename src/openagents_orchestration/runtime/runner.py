...
class OrchestratorRunner:
    ...
    async def run_agent(
        ...
        # Team leader gets a scoped StateBoard for the task subgraph.
        deps_override = None
        sub_board = None
        if agent_type == "team_leader" and self._state_board is not None:
            ...
            if task is not None and task.subgraph is not None:
                sub_board = StateBoard(
                    ...
                    budget=Budget(
                        token_limit=max(1, self._state_board.budget.token_remaining // 4),
                        time_limit_s=max(60.0, self._state_board.budget.time_remaining_s / 4),
                        max_steps=30,
                    ),
                    echo=False,
                    human_channel_service=self._state_board.human_channel_service,
                )
