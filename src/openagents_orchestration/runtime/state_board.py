...
from openagents_orchestration.runtime.human_channel_service import HumanChannelService
        ...
        for key, value in fields.items():
            if hasattr(task, key):
                ...
                if not force and key == "status" and hasattr(value, "value"):
                    allowed_targets = _ALLOWED_TASK_TRANSITIONS.get(task.status, set())
                    if value not in allowed_targets:
                        self.log_event(
                            ...
                            message=(
                                ...
                                f"to {value.value}. "
                                f"Allowed: {[status.value for status in allowed_targets]}"
                            ),
