...
function renderTimeline(result) {
    ...
    const nodes = [
        ...
        workflowNode("Action", "执行边界", {
            action_plan: governance.action_plan,
            action_result: governance.action_result,
        }, governance.action_result?.executed ? "ok" : "warn"),
        workflowNode("Permission", "交付约束", governance.permissions, governance.permissions?.combined?.needs_human ? "warn" : "ok"),
        workflowNode("Evidence", "证据收集", governance.public_evidence, governance.public_evidence?.length ? "ok" : "warn"),
        workflowNode("Verification", "证据校验", governance.verification, governance.verification?.passed ? "ok" : "danger"),
        workflowNode("Safety", "安全检查", governance.safety, governance.safety?.blocked ? "danger" : "ok"),
        workflowNode("Closure", "闭环判断", governance.closure, governance.closure?.closed ? "ok" : "warn"),
