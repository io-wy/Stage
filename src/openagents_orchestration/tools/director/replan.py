...
class ReplanTool(ToolPlugin):
    ...
    name = "replan"
    description = (
        "Replace a failed, stuck, or overly complex task with smaller sub-tasks. "
        "The original task is removed from the plan, the new sub-tasks are inserted, "
        "and any downstream dependencies are automatically rewired to the new tasks.\n\n"
        "# Effects\n"
        "- The target task is removed from the StateBoard.\n"
        "- New sub-tasks are created with unique task_ids (e.g. t_new_1, t_new_2).\n"
        "- Tasks that depended on the old task are updated to depend on the new terminal sub-tasks.\n"
        "- The new tasks are PENDING and ready to be spawned once their own dependencies are met.\n\n"
        "# When to use\n"
        "- A task failed and a smaller scope would likely succeed.\n"
        "- An agent produced partial artifacts but exhausted its step/token budget.\n"
        "- A task mixes too many concerns (e.g. scaffold + API + tests + docs all in one).\n"
        "- The same task has failed twice with the same approach and you need to change strategy.\n"
        "- The task description was ambiguous and needs to be split into clearer steps.\n\n"
        "# When NOT to use\n"
        "- For transient errors (network timeout, 429, temporary API failure) — retry spawn_agent first.\n"
        "- Without first inspecting show_state and reading relevant files — replanning blind usually fails.\n"
        "- When the task is already tiny — replanning a 1-file task is noise.\n"
        "- When the failure is due to a missing dependency that another task will provide — wait for that task.\n\n"
        "# Parameters\n"
        "- task_id (required): the task to replace.\n"
        "- reason (required): why it needs replanning. Be specific: 'step budget exhausted after 3 failed edit attempts', "
        "'mixes scaffolding and business logic', or 'agent kept editing the wrong file'.\n\n"
        "# After replan\n"
        "- Call show_state to see the new tasks and their dependencies.\n"
        "- Spawn the new ready tasks.\n"
        "- Do not assume the old task's artifacts are valid — verify or re-create them.\n\n"
        "Common mistake: replanning with vague reason 'it failed'. The LLM needs concrete context to split the task well."
    )
